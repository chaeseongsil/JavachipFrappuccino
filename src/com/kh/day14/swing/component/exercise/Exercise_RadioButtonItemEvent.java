...
public class Exercise_RadioButtonItemEvent extends JFrame{
	...
	public Exercise_RadioButtonItemEvent() {
		...
		JPanel radioPanel = new JPanel();			// 라디오 버튼을 추가할 팬을 만듦 -> 라디오 팬
		radioPanel.setBackground(Color.LIGHT_GRAY); // 새로 만든 팬의 배경색 변경
		fruits = new JRadioButton[3];				// 라디오 버튼 객체 배열 생성
		imageLabel = new JLabel();					// 라벨 객체 생성
		ButtonGroup btnG = new ButtonGroup();
		
		for(int i = 0; i < fruits.length; i++) {	
			fruits[i] = new JRadioButton(names[i]); // 반복문으로 라디오 버튼 객체 생성해서 배열에 넣음
...
//					}
		...
		c.add(radioPanel, BorderLayout.NORTH);		// 라디오 팬을 컨텐트팬에 추가, 탑에 위치
		c.add(imageLabel, BorderLayout.CENTER);		// 이미지 라벨은 센터에 위치
		imageLabel.setHorizontalAlignment(SwingConstants.CENTER); // 이미지 라벨안의 이미지도 센터로
